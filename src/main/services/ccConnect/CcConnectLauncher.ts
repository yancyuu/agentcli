...
const log = createLogger('CcConnectLauncher');

/** Minimal view of CcConnectClient the launcher needs to probe readiness. */
...
function defaultResolveBinary(): string | null {
  try {
    ...
    const bin = path.join(pkgRoot, 'bin', 'hermit-bridge');
...
export function resolveBridgeCommand(
  ...
  if (!bin) {
    throw new Error(
      'hermit-bridge binary not found — optional dependency not installed ' +
        '(it ships an arm64-darwin build only). Run cc-connect externally or ' +
...
/**
 * Owns launching the cc-connect bridge (via the `hermit-bridge` package) when it
 ...
 * managed cc-connect untouched. Only stop() kills a process THIS launcher started.
 */
export class CcConnectLauncher {
