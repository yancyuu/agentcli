...
<p align="center"><strong>当前版本：v1.9.8</strong></p>
...
- 同时同步写 `~/.hermit/aikey.env`（0600）+ shell hook，新开终端即生效（或 `source ~/.hermit/aikey.env`）。

> 🔒 首次写入前自动把你的**原始** Claude/Codex 配置快照到 `~/.hermit-env.bak`（**只创建一次**，后续认领永不覆盖）。在「token 池 → **一键恢复原始配置**」可随时还原：原本存在的文件回到原内容，token 池新建的文件会被删除，无残留。认领到的 key 是**即焚明文**，不落库、不回显明文。该能力需服务端授权开通（部分账户暂未开放）。
