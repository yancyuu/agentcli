...
openHermit 采用**动态联邦制**协作模型：每个 Agent（Harness）既可以是执行者，也可以是按任务驱动的协调者。Hermit 负责跨团队协作（任务分发、Skills/MCP 配置、使用报告），团队内部由 Honeycomb 等执行运行时负责（todo/loop、代码 review、自组织流转）。
