...
用户在 agentcli 终端菜单走「token 池 → 认领」，客户端按顺序调用 4 个 v3 接口，再把一次性明文 key 写入本地 Claude Code / Codex 配置：
...
### 1.5 写入本地运行时配置
领取到 key 后，客户端写入（先快照原始文件到 `~/.hermit-env.bak` 支持一键恢复）：

| 文件 | 写入内容 |
|---|---|
| `~/.claude/settings.json` | `env.ANTHROPIC_BASE_URL = endpoint`、`env.ANTHROPIC_AUTH_TOKEN = key` |
| `~/.codex/auth.json` | `OPENAI_API_KEY = key` |
| `~/.codex/config.toml` | `base_url = <由 proxy_paths 解析>`、`model`、`wire_api` |
